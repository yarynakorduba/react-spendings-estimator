...
class AddOutlayForm extends React.Component {
  ...
  handleDateChange = ({ target: { value } }) => {
    ...
    this.setState({
      date: date_invalid ? new Date() : value
    })
  }
      ...
      <Fragment>
        ...
        >
          ...
          <input
            ...
            required
          />
          <button type="submit" className="container__input">
            +
          </button>
        </form>
        <div className="error-msg" style={date_invalid ? { opacity: 1 } : { opacity: 0 }}>
          Date should be in the past
        </div>
        <div className="error-msg" style={amount_invalid ? { opacity: 1 } : { opacity: 0 }}>
